...
function spawnPhantom(site, path, streaming, options, cb) {
  ...
  //To calculate website load time
  ...
  if (!streaming) {
    phantomProc.stdout.on('data', function(endTime) {
      loadTime = new Date(endTime) - startTime;
    });
    ...
    phantomProc.on('exit', function(code) {
      if (!calledCallback) {
        ...
        clearTimeout(timeoutID);
        if (code) {
          cb(new Error('PhantomJS exited with return value ' + code));
        } else {
          cb(null, loadTime);
        }
      }
